...
def merge_season_projections(
    ...
    lookup = {}          # (norm_name, team_short) -> Points  [primary]
    lookup_short = {}    # (last_word_of_norm, team_short)     [secondary]
    # Tertiary/quaternary tiers are team-agnostic, so a shared surname (e.g.
    # "Palmer" — Cole Palmer, MID, CHE vs. Alex Palmer, GK, IPS) could
    # otherwise cross-match two unrelated players. Position is required to
    # corroborate these team-agnostic tiers.
    lookup_name = {}     # (norm_name, position) -> Points      [tertiary — team-agnostic]
    lookup_lastword = {} # (last_word, position) -> Points      [quaternary — last resort]
    ...
    subset_candidates = [
        (row["__norm"], str(row["__team_short"]), row["__pos"], row["Points"]) for _, row in sr.iterrows()
    ]
    ...
    for idx in result.index:
        ...
        key = (norm_name, team_short)
        val = lookup.get(key)
        if val is None and last_word:
            val = lookup_short.get((last_word, team_short))
        if val is None and norm_name:
            val = lookup_name.get((norm_name, position))
        if val is None and last_word:
            val = lookup_lastword.get((last_word, position))
        if val is None and norm_name:
            val = _resolve_by_token_subset(norm_name, team_short, position, subset_candidates)
        if val is not None:
...
def compute_early_season_scores(
    ...
    Returns:
        ...
        Season Score, Week1 Score, Fixture Score, GW1 Proj Pts, and Player
        Score columns added.
    """
    result = pool.copy()
